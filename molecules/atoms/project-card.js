...
export default function ProjectCard({ project }) {
  ...
  let projectLinkName = project.linkName;
    ...
    <div className="shadow-lg hover:bg-gray-200 p-4 ">
      <div className="w-full  text-center ">
        ...
        <div className="flex items-center justify-center align-baseline mt-4 top-auto pb-2">
          <Link key={projectName} href={projectLink} passHref>
            <button className=" border border-gray-900 text-blue-600 hover:bg-gray-900 hover:text-blue-100  px-2 py-2 rounded-full mr-2">
              {projectLinkName}
            </button>
          </Link>
        ...
        <Image src={projectImage} alt={projectName} width={400} height={250} />
      </div>
      <hr className="pb-6"></hr>
      <div className="flex  bg-white rounded-lg justify-between ">
        <article className="prose italic">
          <p className="text-gray-700 mt-4 flex-grow ">{projectShortDesc}</p>
        </article>
      </div>
    </div>
  );
